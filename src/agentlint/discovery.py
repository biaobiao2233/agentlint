...
def discover(root: Path, extra_excludes: tuple[str, ...] = ()) -> DiscoveredFiles:
    ...
    for current, dirnames, filenames in os.walk(root, topdown=True, followlinks=False):
        ...
        for dirname in sorted(dirnames):
            ...
            if candidate.is_symlink() or _is_reparse_point(candidate):
                found.skipped.append(_relative(root, candidate) + " (symlink/reparse point)")
                continue
        ...
        for filename in sorted(filenames):
            ...
            if path.is_symlink() or _is_reparse_point(path):
                found.skipped.append(_relative(root, path) + " (symlink/reparse point)")
                continue
            try:
                if path.stat().st_size > MAX_FILE_SIZE:
                    found.skipped.append(_relative(root, path) + " (too large)")
                    continue
            except OSError:
                found.skipped.append(_relative(root, path) + " (unreadable)")
                continue

            if filename in {"AGENTS.md", "AGENTS.override.md"}:
