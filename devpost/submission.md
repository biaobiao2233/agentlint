...
AgentLint runs locally and does not call an API or send scanned configuration to a service. JSON, HTML, and terminal reports use a portable `.` scan root and relative evidence paths by default; URL query values and account names in standard home-directory paths are removed before report serialization. Reports can still contain relative project structure, arbitrary path literals, and redacted excerpts, so review them before sharing. Public screenshots, committed artifacts, and the demo use the portable `examples/unsafe-project` fixture rather than a personal scan output. A skipped supported configuration, skipped link/reparse directory, matched user exclusion, or project instruction byte limit is surfaced as `COVERAGE002` rather than returning PASS. AgentLint safely parses real YAML skill frontmatter and carries an executable positive/negative adversarial phrase corpus, while remaining explicit that its natural-language checks are deterministic lexical heuristics. It cannot prove runtime behavior, intent, or safety and does not replace Codex approval settings, a sandbox, secret management, or human security review.
