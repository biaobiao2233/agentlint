...
刊头、Verdict/说明、统计、Effective Instruction Graph、Capability-to-Authority Map、Findings 的筛选/空态/字段、Coverage/Inventory，以及当前已翻译的规则标题、风险与修复建议会跟随界面语言。为保证可追溯性，原始技术证据仍保持原文：未覆盖翻译的技术消息、路径、行号、摘录、rule/action/node ID 和扫描到的源文本不会被改写；未知 rule ID 使用诚实的后备显示。

## 结果、退出码和报告文件

| 状态/退出码 | 含义 |
| --- | --- |
| `PASS` | 当前没有命中确定性规则；不是“绝对安全”。 |
| `REVIEW` | 没有阻断性错误，但需要人工审阅警告或权限边界。 |
| `BLOCK` | 有至少一项确定性错误，应在安装或分享前处理。 |
| 退出 `0` | 没有达到 `--fail-on` 阈值，或使用了 `--fail-on never`。 |
| 退出 `1` | 发现达到指定阈值；若指定了报告路径，报告仍可能已经写入。 |
| 退出 `2` | 目标、读取或写入报告路径失败。扫描根是符号链接或 Windows reparse point 时也会走此失败路径。 |
...
AgentLint 本地运行，不调用 API，也不会把扫描配置发送到服务。已知的字面量凭据模式会在报告序列化前脱敏，但这是启发式规则，不能保证识别每一种敏感值；报告仍可能包含路径和已经脱敏的摘录，请按仓库敏感级别保存。

真实本地报告会保留实际扫描根目录和相关路径，便于开发者准确定位证据；这不是漏洞，也不会被 CLI 隐藏。对外分享前请先审查报告内容。提交到仓库、公开截图或演示时，请使用可移植的 [`examples/unsafe-project`](examples/unsafe-project) 夹具生成 artifacts，不要上传个人项目的扫描输出。

嵌套目录中的符号链接、Windows junction/reparse point 不会被跟随，会被记录为覆盖范围缺口。扫描根本身若为符号链接或 Windows reparse point 会被拒绝（退出 `2`）；插件或 Skill 组件路径经由这些对象时会被判为无效本地目标。

能力关系图只展示支持的 manifest 引用和解析出的规范性指令事实所支撑的边；它不验证所有可能的运行时关系。
